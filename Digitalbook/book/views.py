...
#设置DJANGO_SETTINGS_MODULE环境变量，指定Django项目的配置文件路径
...
def delete_multiple_books(request):
    if request.method == 'POST':
        ...
        if digit_to_exclude is not None or letter_to_exclude is not None:
            #构造查询条件
            query = Q()

            if digit_to_exclude is not None:
                #匹配id包含特定数字的书籍
                query &= Q(id__contains=digit_to_exclude)
            ...
            #获取符合条件的书籍queryset
            books_to_delete = Book.objects.filter(query)

            if books_to_delete.exists():
                #使用bulk_delete()方法批量删除
                ...
                return JsonResponse({'message': 'No books found matching the criteria.'}, status=200)
        else:
            return JsonResponse({'error': 'No criteria provided for deletion.'}, status=400)
